// functions/privacy.js - Serverless function serving the /privacy page
...
<head>
    ...
    <title>XiaoXi Studio - 隐私政策</title>
    <meta name="description" content="XiaoXi Studio 隐私政策：了解本站如何收集、使用和保护您的个人信息。">
    ...
    <style>
        ...
        #privacy-content-area h1 { font-size: 1.5rem; }
        #privacy-content-area h2 { font-size: 1.2rem; }
        #privacy-content-area h3 { font-size: 1rem; }
        ...
        #privacy-content-area li { margin-bottom: 0.25rem; }
        #privacy-content-area strong { color: #111827; font-weight: 600; }
        #privacy-content-area a { color: #111827; text-decoration: underline; }
        #privacy-content-area hr { border-color: #e5e7eb; margin: 1.5rem 0; }
        #privacy-content-area blockquote {
...
<body class="font-sans antialiased min-h-screen relative overflow-x-hidden bg-grid">
    ...
    <main class="w-full max-w-7xl mx-auto px-4 md:px-8 pb-24 pt-[140px] md:pt-[200px] flex flex-col items-center text-center gap-12">
        ...
        <h1 class="text-4xl sm:text-6xl md:text-[72px] font-extrabold tracking-tighter text-gray-950 max-w-4xl leading-[1.1] mt-8 md:mt-16">
            隐私政策<span class="text-gradient">。</span>
        </h1>
        <p class="text-gray-500 max-w-xl text-sm md:text-base leading-relaxed">
            XiaoXi Studio 极端重视您的数字隐私。本政策向您透明公开本站如何对待您的交互资产。
        </p>

        <!-- Dynamic Markdown content area (hydrated from legal_privacy_content) -->
        <!-- Falls back to static bento grid below if no CMS content is available -->
        <div id="privacy-content-area" class="w-full max-w-5xl text-left mt-2">

            <!-- Static fallback bento grid (shown when no CMS content is fetched) -->
            <div id="privacy-static-grid" class="grid grid-cols-1 md:grid-cols-2 gap-8 w-full text-left">
                
                <!-- Box 01 -->
                <div class="bg-white border border-gray-200/80 rounded-large-card p-8 shadow-sm flex flex-col gap-4">
                    <div class="flex items-center gap-3">
                        <span class="material-symbols-outlined text-gray-950 text-[28px]">database</span>
                        <h3 class="text-lg font-bold text-gray-950">条款 01：数据收集最小化</h3>
                    </div>
                    <p class="text-gray-500 text-sm leading-relaxed">
                        我们仅在您自愿填写前台合作意向表单时收集您的姓名、联系方式（微信/QQ/邮箱）及留言。本站不安装任何第三方侵入性广告追踪组件，亦不保存您的浏览器指纹。
                    </p>
                </div>
                ...
                <div class="bg-white border border-gray-200/80 rounded-large-card p-8 shadow-sm flex flex-col gap-4">
                    <div class="flex items-center gap-3">
                        <span class="material-symbols-outlined text-gray-950 text-[28px]">cloud</span>
                        <h3 class="text-lg font-bold text-gray-950">条款 02：多媒体与托管安全</h3>
                    </div>
                    <p class="text-gray-500 text-sm leading-relaxed">
                        后台管理系统上传的所有个人/客户视频及图片素材均直接存储于 Cloudflare R2 加密存储桶中。我们向您保证，绝不会将任何未经商用授权的交付级视频小样、工程草稿披露给第三方进行未授权展示。
                    </p>
                </div>
                ...
                <div class="bg-white border border-gray-200/80 rounded-large-card p-8 shadow-sm flex flex-col gap-4">
                    <div class="flex items-center gap-3">
                        <span class="material-symbols-outlined text-gray-950 text-[28px]">security</span>
                        <h3 class="text-lg font-bold text-gray-950">条款 03：第三方依赖与免责</h3>
                    </div>
                    <p class="text-gray-500 text-sm leading-relaxed">
                        本站基于 GitHub 与 Cloudflare Pages 构建，数据经全球边缘节点流转与分发。本工作室深入接入了生成式 AI 辅助创作（如 Midjourney、可灵），素材版权与数据隐私边界已严格遵循官方隐私服务协定。
                    </p>
                </div>
                ...
                <div class="bg-white border border-gray-200/80 rounded-large-card p-8 shadow-sm flex flex-col gap-4">
                    <div class="flex items-center gap-3">
                        <span class="material-symbols-outlined text-gray-950 text-[28px]">edit_document</span>
                        <h3 class="text-lg font-bold text-gray-950">条款 04：您的权利与声明更新</h3>
                    </div>
                    <p class="text-gray-500 text-sm leading-relaxed">
                        您可以随时通过合作邮箱联系我更正或物理擦除您提交的意向留言数据。本政策内容最新修订发布于 2026 年 6 月，我们将根据最新的合规性准则进行动态优化调整。
                    </p>
                </div>
    ...
    <script type="module">
        ...
        async function hydratePage() {
            const contentArea = document.getElementById('privacy-content-area');
            const staticGrid  = document.getElementById('privacy-static-grid');
            try {
                const configs = await dataService.getSiteConfigs('legal');
                if (configs.legal_privacy_content && contentArea) {
                    // Hide static fallback grid and replace with rendered markdown
                    if (staticGrid) staticGrid.remove();
                    if (window.marked) {
                        contentArea.innerHTML = marked.parse(configs.legal_privacy_content);
                    } else {
                        contentArea.innerHTML = configs.legal_privacy_content.replace(/\n/g, '<br>');
                    }
                }
            } catch(err) {
                console.warn('[privacy] hydration error:', err);
            }
        }
